...
class LocalRecommendationService implements RecommendationEngine {
  ...
  Future<_BuildResult> _buildDailySet({
    ...
    try {
      feedback = feedbackService == null
          ? RecommendationFeedbackSnapshot.empty()
          : await feedbackService.readSnapshot();
    } catch (_) {
      feedback = RecommendationFeedbackSnapshot.empty();
    }
    final candidates = candidatesPool
        ...
        .where((candidate) {
          if (feedback.isTrackHidden(candidate.stableKey)) return false;
          if (feedback.hasHiddenArtist(candidate.artistKeys)) return false;
          return true;
        })
        .toList();
  ...
  _ScoreResult _scoreCandidate({
    ...
    final retentionSignal =
        ...
        (1 - (skipSignal * 0.7));
    final recentSignal = _recentScore(item.lastPlayedAt, nowMs);
    final completedRecencySignal = _completedRecencyScore(
    ...
    final qualitySignal =
        (retentionSignal * 0.62) +
        ((1 - skipSignal) * 0.26) +
        (progressSignal * 0.12);
    final loyaltySignal =
        (favoriteSignal * 0.34) +
        (fullListenSignal * 0.36) +
        (completedRecencySignal * 0.30);
    final explorationPotential = (semanticMatch * 0.58) + (novelty * 0.42);
    final profileConfidence = _profileConfidence(profile);
    final preferenceBlend = (0.58 + (profileConfidence * 0.24))
        .clamp(0.58, 0.82)
        .toDouble();

    var score =
        (preferenceBlend *
            ((0.43 * affinitySignals) +
                (0.33 * semanticMatch) +
                (0.24 * qualitySignal))) +
        ((1 - preferenceBlend) *
    ...
    final explicitTrackBias = feedback.trackBiasForStableKey(
      candidate.stableKey,
    );
    final artistBias = feedback.bestArtistBias(candidate.artistKeys);
    final tagBias = feedback.bestTagBias(
  ...
  List<_ScoredCandidate> _selectDiverse(
    ...
    bool canSelect(_ScoredCandidate candidate) {
      ...
      if (tag.isNotEmpty && (dominantTagCount[tag] ?? 0) >= maxPerTag) {
        return false;
      }

      final artistKey = candidate.candidate.primaryArtistKey;
      if (artistKey.isNotEmpty &&
          (artistCount[artistKey] ?? 0) >= maxPerArtist) {
        return false;
      }

      final originKey = candidate.candidate.originKey;
      if (originKey.isNotEmpty &&
          (originCount[originKey] ?? 0) >= maxPerOrigin) {
        return false;
      }
    ...
    for (final candidate in ordered) {
      ...
      if (canSelect(candidate)) {
        ...
        final originKey = candidate.candidate.originKey;
        if (originKey.isNotEmpty) {
          originCount[originKey] = (originCount[originKey] ?? 0) + 1;
        }
      } else {
  ...
  double _interactionWeight(MediaItem item, int nowMs) {
    ...
    final completionRecencyWeight =
        _completedRecencyScore(item.lastCompletedAt, nowMs) * 1.2;
  ...
  double _completedRecencyScore(int? lastCompletedAt, int nowMs) {
    final ts = lastCompletedAt ?? 0;
    if (ts <= 0) return 0;
    final ageHours = max(
      0,
      ((nowMs - ts) / const Duration(hours: 1).inMilliseconds).round(),
    ...
    if (ageHours <= 24) return 1;
    if (ageHours <= 24 * 3) return 0.78;
    if (ageHours <= 24 * 7) return 0.58;
    if (ageHours <= 24 * 14) return 0.36;
    return 0.14;
  }

  double _profileConfidence(RecommendationProfile profile) {
    final topGenre = profile.genreWeights.values.fold<double>(0, max);
    final topRegion = profile.regionWeights.values.fold<double>(0, max);
    final topArtist = profile.artistWeights.values.fold<double>(0, max);
    final topOrigin = profile.originWeights.values.fold<double>(0, max);
    final signal =
        (topGenre * 0.30) +
        (topRegion * 0.24) +
        (topArtist * 0.30) +
