...
class CountryCatalog {
  ...
  static final Map<String, CountryOption> _byName = {
    ...
    for (final entry in _englishNames.entries)
      _normalizeText(entry.value): _byCode[entry.key]!,
  };
  ...
  static String? countryNameFromCodeForLocale(
    String? code,
    String languageCode,
  ) {
    final country = findByCode(code);
    if (country == null) return null;
    return countryNameForLocale(country, languageCode);
  }

  static String countryNameForLocale(
    CountryOption country,
    String languageCode,
  ) {
    if (languageCode.trim().toLowerCase().startsWith('en')) {
      return _englishNames[country.code] ?? country.name;
    }
    return country.name;
  }
